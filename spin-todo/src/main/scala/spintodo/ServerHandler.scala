...
object ServerHandler {
  ...
  private def route(request: IncomingRequest): Either[String, JsonResponse] = {
    val path = normalizePath(request.pathWithQuery().toOption.getOrElse("/"))
  ...
  private def readBody(request: IncomingRequest): String = {
    val bytes = (for {
      body <- request.consume().toEither
      inputStream <- body.stream().toEither
    } yield {
      val in = mutable.ArrayBuffer.empty[Byte]
      var eof = false

      while (!eof) {
        inputStream.blockingRead(1024L).toEither match {
          case Right(chunk) =>
  ...
  private def send(outParam: ResponseOutparam, status: Int,
    ...
    val headers = Fields.fromList(Array(
        wm.Tuple2("content-type", contentType.getBytes("UTF-8")))).toEither.getOrElse(Fields())
    val response = OutgoingResponse(headers)

    response.setStatusCode(status.toShort).toEither.getOrElse(
        throw new Error(s"failed to set response status $status"))

    val body = response.body().toEither.getOrElse(
        throw new Error("failed to obtain outgoing response body"))

    ResponseOutparam.set(outParam, new wm.Ok(response))

    val out = body.write().toEither.getOrElse(
        throw new Error("failed to get outgoing stream"))
    out.blockingWriteAndFlush(responseBody.getBytes("UTF-8")).toEither.getOrElse(
        throw new Error("failed to write response body"))

    out.close()
    OutgoingBody.finish(body, java.util.Optional.empty[Trailers]()).toEither.getOrElse(
